...
### [x] 5. Adicionar `setorOperacionalId` à entidade `Quadra` no ER
...
### [x] 6. Especificar o vínculo `Empreiteiro` ↔ `Empreiteira`
...
### [x] 8. Adicionar endpoint de checkout de expediente em SPEC/08
...
### [x] 9. Adicionar endpoints de configuração por obra em SPEC/08
...
### [x] 10. Adicionar endpoint de fila global para AdminOperacional em SPEC/08
...
### [x] 11. Especificar o mecanismo técnico de notificação SLA
...
### [x] 12. Remover spec de integração RH/Folha do sistema

**Decisão (DEC-018):** Integração com sistema de RH/Folha removida do escopo do MVP. O denominador de `REQ-MET-002` passa a ser calculado com base nos operadores cadastrados e ativos no próprio sistema FGR-OPS (`deletadoEm IS NULL`), eliminando a dependência de integração externa.
**Alterações aplicadas:**
- `SPEC/06`: fórmula, denominador, tabela de critérios e artefato de validação — todas as referências a "folha" substituídas por "operadores cadastrados no sistema"
- `SPEC/08`: campo `operadoresNaFolha` → `operadoresCadastrados` em `GET /relatorios/sla`
- `PRD/06`: REQ-MET-002 atualizado para remover referência à folha de pagamento
...
| 11  | Mecanismo técnico de notificação SLA       | IMPORTANTE  | [x]    |
| 12  | Remover spec de integração RH/Folha        | IMPORTANTE  | [x]    |
