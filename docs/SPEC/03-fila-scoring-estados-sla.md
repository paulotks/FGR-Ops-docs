...
- Para demandas originadas de agendamento, o marco zero do SLA é a `dataAgendada` original (`T-0`), e não o momento do aceite explícito pelo operador (DEC-026).
- Se o atendimento ocorrer antes da `dataAgendada`, o tempo de atendimento é considerado zero.
- SLA de demandas roladas (rollover inter-dias) reseta no `expedienteInicio` do dia seguinte. Alertas e escalação de SLA são mantidos durante todo o ciclo inter-dias (DEC-025).
...
### Mecânica do worker `expedienteFim`

O worker `expedienteFim` executa ao fim do expediente de cada obra. Gatilho duplo: checkout do operador ou disparo automático do worker.

### Devolução forçada (`EM_ANDAMENTO` / `PAUSADA`)

Para cada demanda do dia em `EM_ANDAMENTO` ou `PAUSADA` ao fim do expediente:
...
### Rollover de `PENDENTE`

Para cada demanda em `PENDENTE` sem conclusão ao fim do expediente:

1. Campo `rolloverDe` preenchido com a data do dia.
2. Campo `operadorId` limpo (sem operador atribuído).
3. SLA agendado para reset no `expedienteInicio` do dia seguinte.
