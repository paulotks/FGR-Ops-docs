...
> **Amendment 2026-07-21 — filtro de `Setor Operacional` IMPLEMENTADO (DEC-058, `REQ-JOR-002`):** o filtro de jurisdição por Setor descrito acima deixa de ser só intenção de design e passa a estar **implementado** no fluxo least-loaded do MVP (`AutoAllocatorService`/`AllocationCandidateRepository.findEligibleCandidates`), via o vínculo N:M `OperadorSetorOperacional` (`operadorId`, `setorOperacionalId`, `obraId`). Mecânica: operador é candidato elegível se **não tiver nenhum setor vinculado** (lista vazia = atua na **obra inteira**, semântica compat com cadastros pré-existentes) **ou** se tiver o `setorOperacionalId` da demanda entre os seus. Demanda legada sem `setorOperacionalId` (NULL) não sofre o filtro. Quando não há candidato do setor, a demanda fica **órfã, sem fallback automático** para "obra inteira" (log estruturado com `motivo: SEM_COBERTURA_SETOR` + `candidatosForaSetor`); a alocação manual (Regra Zero, DEC-001, passo 1 acima) segue bypassando o filtro. O broadcast de demandas `AGENDADA` (DEC-026 Q1) foi reavaliado e **mantido sem filtro de setor** — vale só para a distribuição automática instantânea. A incompatibilidade equipamento/serviço (segunda cláusula deste item, filtro por `OperadorTipoMaquinario`) já era hard-filtrada no auto-allocator desde antes desta amendment — DEC-054 estendeu esse mesmo check aos caminhos **manuais** (`POST /demandas` com `operadorId` e `POST /demandas/:id/alocar`), não ao automático; **scoring e adjacência** (item 4 abaixo) seguem **desligados, pós-MVP** — este filtro de setor é inclusive pré-requisito deles e fica pronto esperando. Detalhe: `docs/audit/decisions-log.md` DEC-058; design `docs/superpowers/specs/2026-07-21-operador-setores-atuacao-design.md`.
...
> **Amendment 2026-07-19 (DEC-057, `REQ-ACE-004`):** `DemandaLog` ganha a coluna
> `perfil` (`NVARCHAR(30) NULL`) — o papel do ator (valor string do enum `Perfil`,
> ex. `TOWER_OPERATOR`) no momento da ação, para relatórios de auditoria. É gravada
> em toda escrita USER-driven (as 6 transições de estado acima, `alocar` manual,
> `reordenar`, criação manual). Ações `SISTEMA` (`rollover`, `devolver_fim_expediente`,
> auto-alocação — `userId=null`) e linhas históricas anteriores a esta migration
> seguem com `perfil=null`. `userId` continua sendo a referência do responsável;
> o nome do ator não é snapshotado, é resolvido por join sob demanda. Contrato
> completo do campo em [02-modelo-dados.md](02-modelo-dados.md#ações-registradas-em-demandalog).
