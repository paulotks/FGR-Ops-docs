...
- **Aggregates e Entities**: `Demanda` é o Aggregate Root, compondo entidades filhas como `DemandaLog` e amparando agrupamentos via `DemandaGrupo`. `Maquinario` e `Operador` são entidades operacionais essenciais. O cadastro de recursos abrange `Maquinario` (com atributos `placa`, `TipoMaquinario`, `Servico` vinculado e `proprietarioTipo` FGR/EMPREITEIRA) e `Ajudante` (recurso humano vinculado à obra sem credencial própria no sistema). O modelo de dados completo destas entidades está detalhado em [02-modelo-dados.md](02-modelo-dados.md).
