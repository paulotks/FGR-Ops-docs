...
## DEC-057 — `DemandaLog` ganha coluna `perfil` (papel do ator, para auditoria)

- **Estado:** Ativo
- **Data:** 2026-07-19
- **Participantes:** Paulo (FGR), Engenharia
- **Contexto:** a branch `feat/demanda-log-perfil-fila-cancel` (`REQ-ACE-004` + `REQ-FUNC-009`) trabalha duas frentes: (A) perfil do ator no `DemandaLog` e (B) cancelar demanda diretamente na tela de Fila. Esta DEC cobre a frente A. Hoje o `DemandaLog` grava `userId` (o responsável pela ação) e `ator` (`USER`/`SISTEMA`), mas não o **papel/perfil** que esse usuário tinha no momento da ação — informação necessária para relatórios de auditoria futuros ("quem fez o quê, com qual papel"), já que o `perfil` de um `User` pode mudar ao longo do tempo e o log precisa refletir o papel vigente no instante do evento, não o atual.
- **Opções consideradas:**
  - A) Snapshot completo do ator (nome + perfil) em cada linha do log — REJEITADA: nome já é resolvível por join via `userId` (o `User` não é apagado, só soft-delete em outras entidades); duplicar nome é redundância sem ganho e complica LGPD (nome congelado sobrevive a alterações cadastrais).
  - B) Coluna `perfil` nullable, populada apenas nas escritas USER-driven; `userId` segue como referência única do responsável — **ESCOLHIDA**.
  - C) Tabela de histórico de perfil por usuário (`UserPerfilHistorico`) consultada sob demanda — REJEITADA no MVP: complexidade desproporcional ao ganho; o `perfil` denormalizado no próprio log já resolve o caso de uso de auditoria sem nova entidade.
- **Decisão:** `DemandaLog` ganha a coluna `perfil NVARCHAR(30) NULL`, no mesmo padrão já usado em `AuthAuditLog`/`AuditLogCrossTenant` (string do enum `Perfil`, ex. `TOWER_OPERATOR`). É preenchida em toda escrita USER-driven: as 6 transições de estado (`PATCH /demandas/:id/estado`), alocação manual (`alocar`), reordenação (`reordenar`), criação manual da demanda e a restauração da ordem FIFO da fila (`POST /demandas/fila/restaurar` — `restaurar_ordem_fila`, ação de gestor). Ações `SISTEMA` (`rollover`, `devolver_fim_expediente`, auto-alocação na criação — todas com `userId=null`) e linhas históricas anteriores a esta migration permanecem com `perfil=null`.

  > **Amendment 2026-07-19 (whole-branch review, mesma PR):** `restaurar_ordem_fila` foi incluída na lista USER-driven acima após o review de branch identificar que ela grava `ator: 'USER'`+`userId`, mas vinha sem `perfil` (o design inicial a classificara como possível SISTEMA). Corrigido no commit `feat(demanda): grava perfil do ator no restaurar_ordem_fila` da mesma branch — nenhuma ação de USER fica sem `perfil`. `userId` continua sendo a referência canônica do responsável pela ação; o **nome** do ator não é snapshotado — é resolvido por join sob demanda quando necessário (ex.: tela de auditoria).
- **Justificativa:** `REQ-ACE-004` exige audit log rastreável em modificações gerenciais; o perfil do ator é um atributo de auditoria de baixo custo (uma coluna nullable, sem nova entidade) que fecha a lacuna "papel no momento do ato" sem introduzir redundância de identidade (nome) nem uma tabela de histórico de perfil fora de escopo do MVP.
- **Nota de numeração:** contador `fgr-ops-docs/CLAUDE.md` em **DEC-057** (log append-only termina em DEC-056). Sem colisão de draft (DEC redigida direto). Contador bumpado **DEC-057 → DEC-058**.
- **SPECs/REQ-IDs afetados:** `SPEC/02-modelo-dados` (ER de `DemandaLog` ganha `perfil`; seção "Ações registradas em DemandaLog"); `SPEC/03-fila-scoring-estados-sla` (amendment na cadeia de contrato do `DemandaLog`); `REQ-ACE-004`, `REQ-FUNC-009`; relacionadas: amendment 2026-07-11 known-debt PR #85 (contrato atual de `DemandaLog`). **Plano (code repo):** `.superpowers/sdd/task-5-brief.md` (branch `feat/demanda-log-perfil-fila-cancel`).

---

## DEC-058 — Setores de atuação do Operador: vínculo N:M `OperadorSetorOperacional` fecha o Hard Filter de jurisdição (`REQ-JOR-002`)

- **Estado:** Ativo
- **Data:** 2026-07-21
- **Participantes:** Paulo (FGR), Engenharia
- **Contexto:** `REQ-JOR-002` sempre definiu o `Setor Operacional` como filtro **eliminatório** de jurisdição logística, e o Hard Filter da `SPEC/03` (passo 2) já descrevia "demandas saem da fila elegível do operador se pertencerem a Setor Operacional distinto" — mas o vínculo Operador↔Setor nunca existiu no modelo: a `Demanda` nasce com `setorOperacionalId` (derivado da quadra/local externo de origem, DEC-005), porém `Operador` não tinha nenhuma associação a setor. Sem essa perna, o filtro era inimplementável e caiu junto com o desligamento do scoring/adjacência (MVP-15jul, DEC-040 — Tower Operator orquestra a fila manualmente via `AutoAllocatorService.selectLeastLoaded`, zero noção de localização). Sintoma relatado em campo: operador recebendo demanda de quadra/lote do outro lado do condomínio; a mitigação manual observada ("alocar o operador numa rua") é exatamente a intenção original do `SetorOperacional` (configurável por obra, no tamanho de uma rua — não se criou um segundo mecanismo de jurisdição). Design: `docs/superpowers/specs/2026-07-21-operador-setores-atuacao-design.md` (brainstorm 2026-07-21).
- **Opções consideradas:**
  - A) Cardinalidade 1:1 (`Operador.setorOperacionalId` único) — REJEITADA: operadores reais atuam em mais de um setor/rua; forçaria 1 só ou duplicação de cadastro.
  - B) Cardinalidade N:M via junction `OperadorSetorOperacional` (molde `OperadorMaquinario`), lista vazia = atua na obra inteira — **ESCOLHIDA**: compat total com dados atuais (nenhum operador cadastrado hoje tem vínculo, logo nenhum perde cobertura no dia do deploy) e espelha o padrão já existente de `OperadorTipoMaquinario`/`OperadorMaquinario`.
  - C) Fallback automático para obra inteira quando o setor não tem operador elegível — REJEITADA: reintroduz o problema original de forma silenciosa (demanda cai em quem não deveria atendê-la); a demanda deve ficar órfã e visível para intervenção humana.
  - D) Setor escolhido pelo próprio operador no check-in (por turno) — REJEITADA: auto-alocação decidida pelo próprio operador, mudança invasiva no fluxo de campo; jurisdição é decisão de gestão, não do operador.
  - E) Edição do vínculo restrita a `AdminOperacional`/`SuperAdmin` (sem `TOWER_OPERATOR`) — REJEITADA: realocar operador de rua conforme a frente de obra avança é tarefa do dia-a-dia de quem orquestra a fila (o Tower Operator); restringir forçaria um admin como intermediário para uma operação rotineira.
- **Decisão:**
  1. **Modelo de dados:** nova tabela N:M `OperadorSetorOperacional { operadorId, setorOperacionalId, obraId }` (PK composta `[operadorId, setorOperacionalId]`), espelho de `OperadorMaquinario` — tenant **direct** (`obraId` próprio na linha, não indirect como `OperadorTipoMaquinario`), pois `SetorOperacional` é tenant-scoped como `Maquinario` e o `obraId` na linha ativa a barreira `enforceObraIdOnData` do tenant guard sem wiring indireto. **Lista vazia = operador atua na obra inteira** (semântica canônica, compat total com cadastros pré-existentes).
  2. **Hard filter só no fluxo automático:** `AllocationCandidateRepository.findEligibleCandidates` ganha condição adicional no `AND` (mesmo molde do filtro de tipo): candidato é elegível se **não tem nenhum setor cadastrado** (obra inteira) **ou** tem `demanda.setorOperacionalId` entre seus setores. Demandas legadas sem `setorOperacionalId` (NULL) **não** sofrem o filtro (comportamento atual preservado). Alocação manual (`POST /demandas/:id/alocar` e pré-alocação `operadorId?` no create) **bypassa** — Regra Zero (DEC-001) permanece soberana, sem guard novo de setor no caminho manual.
  3. **Órfã sem fallback:** quando não há candidato do setor da demanda, a demanda fica **órfã** (nenhum operador é atribuído automaticamente) — sem fallback silencioso para "obra inteira". O log estruturado de órfã é enriquecido com `motivo: SEM_OPERADOR_EM_EXPEDIENTE | SEM_HABILITADO_NO_TIPO | SEM_COBERTURA_SETOR`; no motivo `SEM_COBERTURA_SETOR`, o log carrega `candidatosForaSetor` (contagem de aptos por expediente+tipo, mas fora do setor) e `setorOperacionalId` da demanda.
  4. **DEC-026 Q1 reavaliada e MANTIDA:** o broadcast de demandas `AGENDADA` por `TipoMaquinario` segue **sem filtro de setor** — a demanda agendada é planejada com antecedência e o operador pode se deslocar até o horário do slot; jurisdição por setor se aplica só à distribuição automática instantânea (least-loaded).
  5. **Carve-out `TOWER_OPERATOR` (molde DEC-040):** endpoint dedicado `PATCH /operadores/:id/setores` (`{ setoresIds: string[] }`, replace-whole-set, dedup) é liberado a `AdminOperacional`, `SuperAdmin` **e `TOWER_OPERATOR`** — carve-out cirúrgico, mesma obra do JWT, **sem bypass de tenant** (mesmo padrão do carve-out de criação de Demanda da DEC-040). `TOWER_OPERATOR` continua **sem** acesso ao CRUD completo de Operador (`POST/PATCH /operadores` seguem restritos a `AdminOperacional`/`SuperAdmin`); o carve-out é estrito ao vínculo de setores, refletindo que realocar operador de rua é operação rotineira de quem orquestra a fila.
  6. **Read-side de gestão:** `GET /demandas/kanban` (`operadores[].setoresIds`, card ganha `setorOperacionalId`/`setorNome`/`semCoberturaSetor`) e `GET /demandas/fila-gestao` (itens ganham `setorNome`/`semCoberturaSetor`) expõem a cobertura de setor para o modal de alocar e o badge de órfã. `semCoberturaSetor` é **computado no read-side, nunca persistido** — existe operador em expediente habilitado no tipo, mas nenhum do setor da demanda; aplica-se só a demanda `PENDENTE` sem operador alocado. Por não ser persistido, nunca fica stale quando alguém do setor faz check-in.
  7. `DELETE /obras/:id/setores/:setorId` (hard-delete, `409 REC-002`, DEC-045) passa a checar também **operadores vinculados** (`OperadorSetorOperacional`) na lista de dependências que bloqueiam a exclusão, ao lado de quadras e locais externos.
- **Justificativa:** o filtro de jurisdição por `Setor Operacional` sempre foi parte do design de `REQ-JOR-002`/Hard Filter (`SPEC/03` passo 2); faltava apenas o vínculo Operador↔Setor para torná-lo executável. Fechar essa lacuna resolve o sintoma relatado em campo (operador recebendo demanda geograficamente distante) sem esperar pela reativação do scoring/adjacência (pós-MVP) — o filtro de setor é inclusive pré-requisito desse motor e fica pronto esperando. A cardinalidade N:M com lista-vazia-= obra-inteira evita quebrar cadastros existentes; a ausência de fallback automático evita reintroduzir o problema original de forma silenciosa; e o carve-out do `TOWER_OPERATOR` segue o precedente já ratificado pela DEC-040 (mesma obra, sem bypass de tenant, ação rotineira do orquestrador da fila).
- **Nota de numeração:** contador `fgr-ops-docs/CLAUDE.md` em **DEC-058** (log append-only termina em DEC-057). Sem colisão de draft (DEC redigida direto). Contador bumpado **DEC-058 → DEC-059**.
- **SPECs/REQ-IDs afetados:** `SPEC/03-fila-scoring-estados-sla` (amendment 2026-07-21 no passo 2, Hard Filter — filtro de Setor Operacional passa a IMPLEMENTADO no fluxo least-loaded do MVP); `SPEC/04-rbac-permissoes` (matriz ganha `machinery:operador-setor:update` + nota de rodapé + bullet na subseção "Perfil TOWER_OPERATOR"); `SPEC/08-api-contratos` (`POST/PATCH /operadores` ganham `setoresIds`/`setoresAtuacao`; endpoint novo `PATCH /operadores/:id/setores`; `DELETE /obras/:id/setores/:setorId` amenda a lista de dependências do `409 REC-002`; `GET /demandas/kanban` e `GET /demandas/fila-gestao` ganham os campos de cobertura de setor); `REQ-JOR-002`; relacionadas: `DEC-005` (setor derivado da origem da demanda), `DEC-001` (Regra Zero — alocação manual bypassa), `DEC-026` (Q1 broadcast de agendadas, reavaliada e mantida), `DEC-040` (molde do carve-out `TOWER_OPERATOR`), `DEC-045` (dependências do hard-delete de setor). **Design (code repo):** `docs/superpowers/specs/2026-07-21-operador-setores-atuacao-design.md`.
