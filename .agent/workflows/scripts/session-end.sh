#!/usr/bin/env bash
# session-end.sh
# Adaptado de .claude/hooks/session-end.sh para uso em .agent/workflows/
# Executar a partir da raiz do repositório: bash .agent/workflows/scripts/session-end.sh
...
# --- Calcular próxima DEC disponível ---
NEXT_DEC=$(grep -oE 'DEC-[0-9]+' "$DECISIONS_LOG" 2>/dev/null | grep -oE '[0-9]+' | sort -n | tail -1 | awk '{printf "DEC-%03d\n", $1+1}' || echo "DEC-019")
