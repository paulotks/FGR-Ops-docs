...
# Roda após edições em docs/PRD/** ou docs/SPEC/**
# Verifica integridade de rastreabilidade entre PRD, SPEC e traceability.md
...
# Extrair REQ-IDs do _index.md (padrão REQ-PREFIX-NNN)
...
while IFS= read -r spec_file; do
  # Extrair todos os REQ-IDs citados neste arquivo SPEC
  ...
  # Verificar se seções que citam REQ têm bloco Rastreio PRD:
  # Simplificado: se o arquivo cita REQ-IDs, deve ter pelo menos um bloco Rastreio PRD:
...
while IFS= read -r prd_file; do
  # Extrair links → SPEC: path#anchor
  while IFS= read -r link; do
    # Extrair o caminho após "→ SPEC: " ou "SPEC:"
    rel_path=$(echo "$link" | grep -oE '\(\.\.?/SPEC/[^)#]+#[^)]+\)' | tr -d '()' || true)
    if [[ -z "$rel_path" ]]; then
      rel_path=$(echo "$link" | grep -oE '\.\./SPEC/[^ ]+#[^ ]+' || true)
    fi
    if [[ -n "$rel_path" ]]; then
      # Separar arquivo e anchor
      file_part="${rel_path%%#*}"
      anchor_part="${rel_path##*#}"
      # Resolver path relativo a partir de docs/PRD/
...
if [[ -f "$TRACEABILITY" ]]; then
  for req_id in $REGISTERED_IDS; do
    if ! grep -qF "$req_id" "$TRACEABILITY"; then
      # Verificar se está coberto por um grupo (ex: REQ-CTX-*)
